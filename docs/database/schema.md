...
erDiagram
  ...
  POST_DRAFTS {
    text post_localization_id PK, FK
    text title
    text excerpt
    json content_json
    text draft_token
  ...
  POST_REVISION_MEDIA {
    ...
    text asset_r2_key
    integer asset_width
    integer asset_height
    text asset_caption
    text asset_creator_name
    text asset_source_url
    text asset_license_label
    text asset_license_url
  }
  ...
  POST_LOCALIZATIONS ||--o| POST_DRAFTS : "has mutable draft"
  MEDIA_ASSETS |o--o{ POST_DRAFTS : "optional social preview image"
...
| Draft                | A localization has at most one mutable `post_drafts` row. Autosave uses opaque-token compare-and-swap; revisions are published snapshots only. See ADR-0032 and ADR-0035.                                                                                                                                                                                                                                                                                            |
...
| Slug history         | `post_localization_slug_history` append-only maps retired slugs to their localization for `301` redirects, unique on `(locale, old_slug)`. Redirect resolution joins that localization's current slug, so A→B→C remains one hop without rewriting history. Retired slugs are reserved permanently; reuse prevention is enforced in application code, not by a constraint. See ADR-0010 and ADR-0038.                                                                 |
...
| Rich text            | Drafts accept only the strict editable document. Publishing copies it into a separate strict representation that may add escaped Shiki token data to code blocks; clients cannot submit those tokens (ADR-0037).                                                                                                                                                                                                                                                     |
| Localized SEO/OG     | Drafts clone all editable SEO/OG fields from the published revision on first save; editor-core exposes only title, excerpt, and content.                                                                                                                                                                                                                                                                                                                             |
| Analysis             | Analysis posts may reference one `games` row. Games never store numeric scores.                                                                                                                                                                                                                                                                                                                                                                                      |
| Media                | `media_assets.r2_key` stores an immutable WebP object key. Inspected dimensions, size, and content type are recorded after the bounded upload; `alt_text` is canonical, with an explicit empty value representing a decorative asset (ADR-0036).                                                                                                                                                                                                                     |
...
| Inline media         | Publishing materializes `post_revision_media` from the reviewed document in one bounded statement in the same D1 batch as its revision and published pointer. The row snapshots immutable R2 address/dimensions and all visible attribution fields; public reads never join mutable asset metadata. HTTP(S) is the only persisted/rendered external URL scheme. Referenced assets cannot be deleted because the placement FK is restrictive.                         |
