...
import {
  ...
  callRetryLocalizationPurge,
  callRetryPublicationPurge,
  callUnpublish,
...
type State = 'idle' | 'pending' | 'error' | 'acknowledgement-error' | 'cache-warning';
...
export function AdminReview({ items, detail, page, pageSize, total }: Props) {
  ...
  const finish = (warning = false) => {
    if (warning) setState('cache-warning');
  ...
  const publish = async () => {
    ...
    if (data) {
      if (data.status === 'published-with-cache-warning') finish(true);
      else {
  ...
  const retry = async () => {
    if (!detail?.draftToken) return;
    const data = operationId.current
      ? await run(() =>
          callRetryPublicationPurge({
            ...mutation!,
            draftToken: detail.draftToken!,
      ...
      : await run(() => callRetryLocalizationPurge(mutation!));
    if (data) finish();
  };
            ...
            {items.map((item) => (
              <li key={item.localizationId}>
                <a
                  ...
                  aria-current={detail?.localizationId === item.localizationId ? 'page' : undefined}
        ...
        {!detail ? (
          ...
          <form className="grid gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                {t(
                  detail.status === 'published' ? 'admin.review.republish' : 'admin.review.publish'
                )}
              </Button>
              <Button
                type="button"
                ...
                onClick={() =>
                  void run(() => callUnpublish(mutation!)).then((data) => {
                    if (data) finish((data as { status?: string }).status?.includes('warning'));
                  })
                }
              >
                ...
                onClick={(event) => {
                  ...
                  void run(() =>
                    ...
                    if (isRenameLocalizationRejection(data)) {
                      ...
                      finish((data as { status?: string }).status?.includes('warning'));
                    }
                  });
                }}
              >
